...
public class PipelineSyntaxTest {
    ...
    public void testSnippetizerDefaults() throws Exception {
        ...
        ContainerAdapter tc = new Tomcat8xAdapter("http://example.com", "id");
        DeployPublisher dp = new DeployPublisher(Collections.singletonList(tc), "app.war");

        t.assertRoundTrip(new CoreStep(dp), "deploy adapters: [tomcat8(credentialsId: 'id', url: 'http://example.com')], war: 'app.war'");
    }
    ...
    public void testSnippetizerNonDefault() throws Exception {
        ...
        t.assertRoundTrip(new CoreStep(dp), "deploy adapters: [tomcat8(credentialsId: 'id', url: 'http://example.com')], contextPath: 'my-app', onFailure: false, war: 'app.war'");
    }
