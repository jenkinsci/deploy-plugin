...
class Tomcat8xAdapterTest {
    ...
    void setUp(JenkinsRule rule) throws Exception {
        ...
        adapter = new  Tomcat8xAdapter(url, c.getId(), null, null);
        adapter.loadCredentials(/* temp project to avoid npe */ jenkinsRule.createFreeStyleProject());
    }

    @Test
    void testContainerId() {
        assertEquals(adapter.getContainerId(), new Tomcat8xRemoteContainer(null).getId());
    }
    ...
    void testDeploy() throws Exception {
        ...
        project.getPublishersList().add(new DeployPublisher(Collections.singletonList(this.adapter), "simple.war"));
